...
// Stack represents a stack in Abg, a set of instructions to build a container.
type Stack struct {
	Name       string
	Base       string
	Packages   []string
	PkgManager string
	BuiltIn    bool // If true, the stack is built-in (stored in /usr/share/abg/stacks) and cannot be removed by the user
}
...
func LoadStack(name string) (*Stack, error) {
	usrStackFile := SelectYamlFile(abg.Cnf.UserStacksPath, name)
	stack, err := LoadStackFromPath(usrStackFile)
	if err != nil {
		stackFile := SelectYamlFile(abg.Cnf.StacksPath, name)
...
func (stack *Stack) Save() error {
	...
	filePath := SelectYamlFile(abg.Cnf.UserStacksPath, stack.Name)
...
func ListStacks() []*Stack {
	...
	stacksFromEtc := listStacksFromPath(abg.Cnf.UserStacksPath)
	stacks = append(stacks, stacksFromEtc...)

	if abg.Cnf.UserStacksPath == abg.Cnf.StacksPath {
		// user install
		return stacks
	}

	stacksFromShare := listStacksFromPath(abg.Cnf.StacksPath)
...
// this func does not return an error, since Abg is meant to be portable and
