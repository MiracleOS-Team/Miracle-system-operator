...
func (d *dbox) RunCommand(command string, args []string, engineFlags []string, useEngine bool, captureOutput bool, muteOutput bool, rootFull bool, detachedMode bool) ([]byte, error) {
	...
	if d.Engine == "podman" {
		...
		// cmd.Env = append(cmd.Env, "XDG_DATA_HOME="+abg.Cnf.ApxStoragePath)
	} else if d.Engine == "docker" {
		cmd.Env = append(cmd.Env, "DOCKER_STORAGE_DRIVER="+abg.Cnf.StorageDriver)
		// cmd.Env = append(cmd.Env, "DOCKER_DATA_ROOT="+abg.Cnf.ApxStoragePath)
	}
	...
	// NOTE: the root flag is not being used by the Abg CLI, but it's useful
	//		 for those using Abg as a library, e.g. VSO.
	if rootFull && !useEngine {
