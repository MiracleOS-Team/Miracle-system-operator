...
// PkgManager represents a package manager in Abg, a set of instructions to handle a package manager.
type PkgManager struct {
	...
	// /usr/share/abg/pkg-managers) and cannot be removed by the user
	BuiltIn bool
}
...
func LoadPkgManager(name string) (*PkgManager, error) {
	userPkgFile := SelectYamlFile(abg.Cnf.UserPkgManagersPath, name)
	pkgManager, err := loadPkgManagerFromPath(userPkgFile)
	if err != nil {
		pkgFile := SelectYamlFile(abg.Cnf.PkgManagersPath, name)
...
func (pkgManager *PkgManager) Save() error {
	...
	filePath := SelectYamlFile(abg.Cnf.UserPkgManagersPath, pkgManager.Name)
...
func (pkgManager *PkgManager) GenCmd(cmd string, args ...string) []string {
	...
	if pkgManager.Model == 0 || pkgManager.Model == 1 {
		...
		fmt.Println("!!! DEPRECATION WARNING: Model 1 will be removed in the future, please update your Abg package manager to use model 2.")
...
func ListPkgManagers() []*PkgManager {
	...
	pkgManagersFromEtc := listPkgManagersFromPath(abg.Cnf.UserPkgManagersPath)
	...
	pkgManagersFromShare := listPkgManagersFromPath(abg.Cnf.PkgManagersPath)
